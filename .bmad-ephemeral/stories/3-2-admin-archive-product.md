...
- [ ] Confirm the `products` table has the `is_archived` boolean column with a default of `false` (this was included in the schema from the Epic 1 Tech Spec).

### Task 2: Update Backend API
- [ ] Modify the `GET /api/products` controller to filter results where `is_archived` is `false`. This will hide archived products from the public catalog.
- [ ] The `PUT /api/products/:id` endpoint should be updated to allow changing the `is_archived` status.

### Task 3: Frontend UI
- [ ] In `AdminDashboard.jsx`, add an "Archive" button to each product row in the table.
- [ ] Add a "Show Archived" toggle or checkbox. When checked, the product list should fetch and display all products, including archived ones.
- [ ] The "Archive" button should change to "Unarchive" for products where `is_archived` is `true`.
- [ ] Clicking the "Archive"/"Unarchive" button should call the `PUT /api/products/:id` endpoint to toggle the `is_archived` flag.
